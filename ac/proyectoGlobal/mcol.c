...
// === PARA PARALELIZAR ===
// ========================
void procesar_peticiones (double mat[NFIL][NCOL], int *columnas, int cant, 

  double *resultados){
  int i, j, columna;
  double y, suma;
  #pragma omp parallel for private(i, columna, y, suma)
  for (j=0; j<cant; j++){
  columna = columnas[j];
  suma = 0.0;
  ...
  strcpy (fsol, argv[1]);
  strcat (fsol, "_sol"); // la version paralela: _sol_p
